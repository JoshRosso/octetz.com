...
Based on this architecture, we have the database configured to allow 150 parallel connections. We've also setup Jetty to include 100 available threads. Ensuring the application can process 100 parallel requests at a given time.
...
After all this work, we are at the same conclusion stated in paragraph 1. *DB can pooling provide massive gains to our application's throughput capabilities*. Let's take a moment to review our before and after:
